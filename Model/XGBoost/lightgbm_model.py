...
for i in range(len(np_test_output)):
    if np_test_output[i] == 1:
        level_count += 1
        if np_test_output[i] == ans[i]:
            level_correct_count += 1
            total_correct_count += 1
    elif np_test_output[i] == 0:
        no_level_count += 1
        if np_test_output[i] == ans[i]:
...
TP = FN = FP = TN = 0
for i in range(len(np_test_output)):
    if np_test_output[i] == 1 and ans[i] == 1:
        TP += 1
    elif np_test_output[i] == 1 and ans[i] == 0:
        FN += 1
    elif np_test_output[i] == 0 and ans[i] == 1:
        FP += 1
    elif np_test_output[i] == 0 and ans[i] == 0:
        TN += 1
precision = TP / (TP + FP)
recall = TP / (TP + FN)
